...
var mapOptions = {
    ...
    layers: [wellPointsLayerGroup, censusTractsLayerGroup, nitrateRatesIDWLayerGroup, joinedCancerNitrateRatesIDWLayerGroup, regressionResidualsLayerGroup]
};
...
// You must use Layer Groups, Leaflet will not read plain objects properties.
...
var easyPrint= L.easyPrint({
    ...
    hideClasses: ['#sidebar','.overlays','#leaflet-control-layers','#layerList','#zoomControl'],
    addClasses: ['#legend','myMenu','myWrapper','logoWrapper'],
    exportOnly: true,
    filename: 'Wisconsin_Cancer_Nitrates',
...
//printPlugin.printMap('A4Portrait', 'Wisconsin_Cancer_Nitrates');

// Use JQuery's getJSON() method to load the census tract and cancer rate data asynchronously
$.getJSON("data/cancer_tracts_84.json", function (data) {
...
function drawCensusTracts() {
    ...
    censusTracts.eachLayer(function (layer) {
        ...
        var popup = "<b>Tract ID: </b>" + (layer.feature.properties.GEOID10)
        +"<br><b>Cancer Rate: </b>" + (layer.feature.properties.canrate * 1).toLocaleString() + "%";
...
function drawCancerRatesLegend(breaks) {
    ...
    legend.onAdd = function () {
        ...
        div.innerHTML = "<h3><b>Cancer Rate </b><i>(%)</i></h3>";
        ...
        for (var i = 0; i < breaks.length; i++) {
            ...
            div.innerHTML +=
                ...
                '<label>' + parseFloat(breaks[i][0] * 1).toFixed(2).toLocaleString() + '% &mdash; ' +
                parseFloat(breaks[i][1] * 1).toFixed(2).toLocaleString() + '%</label>';
...
function drawNitrateRatesLegend(breaks) {
    ...
    legend.onAdd = function () {
        ...
        // First append an <h3> heading tag to the div holding the current attribute
        div.innerHTML = "<h3><b>Nitrate Concentration </b><i>(ppm)</i></h3>";
...
function getUIActions() {
    ...
    submit.on('click', function (e) {
        ...
        console.log("Processing requested parameters now...");
    ...
    reset.on('click', function (e) {
        ...
        console.log("Resetting parameters now...");
...
function submitParameters() {
    ...
    if (isNaN(hexbinArea) || hexbinArea < 3 || hexbinArea > 50) {
        window.alert("Please enter a value between 3 and 50!");
        $('#hexbinArea').val(7);
        resetParameters();
        return;
} else
     ...
     if (isNaN(distanceDecayCoefficient) || distanceDecayCoefficient < 1 || distanceDecayCoefficient > 100) {
        window.alert("Please enter a value between 1 and 100!");
        $('#distanceDecayCoefficient').val(1);
        resetParameters();
        return;
    }
...
function interpolateNitrateRates(distanceDecayCoefficient, hexbinArea) {
    ...
    var options = {
        ...
        weight: distanceDecayCoefficient // distance decay coefficient (k)
    };
    ...
    nitrateRatesHexbins = L.geoJson(nitrateRatesHexbinsTurf, {
        ...
        style: function (feature) {
            return {
                ...
                weight: 0.0, // Stroke Weight
                fillOpacity: 0.5, // Override the default fill opacity
                opacity: 0.0 // Border opacity
            };
        }
...
function joinCancerRatesToNitrateInterpolation(distanceDecayCoefficient, hexbinArea) {
    ...
    for (var i in collectedFeaturesHexbinsTurf.features) {
        ...
        // Get the mean cancer rate (sum / number of features in the array)
        var canrateArrayAvg = canrateArraySum / canrateArray.length;

        // Add the mean cancer rate to the canrate property of the current hexbin
    ...
    collectedFeaturesHexbins.eachLayer(function (layer) {
        ...
        // Build the popup for the feature
        var popup = "<b>Cancer Rate: </b>" + (layer.feature.properties.canrate * 1).toFixed(2).toLocaleString() + "%";
...
function calculateLinearRegression(collectedFeaturesHexbinsTurf) {
    ...
    regressionFeaturesHexbins.eachLayer(function (layer) {
        ...
        var popup = "<b>Nitrate Concentration: </b>" + layer.feature.properties.nitr_ran.toFixed(2) + " ppm" + "<br/>" +
            "<b>Observed Cancer Rate: </b>" + (layer.feature.properties.canrate * 1).toFixed(2).toLocaleString() + "%" + "<br/>" +
            "<b>Predicted Cancer Rate: </b>" + (layer.feature.properties.predictedCancerRate * 1).toFixed(2).toLocaleString() + "%";
...
function drawRegressionResidualsLegend(breaks) {
    ...
    legend.onAdd = function () {
        ...
        // First append an <h3> heading tag to the div holding the current attribute
        div.innerHTML = "<h3><b>Regression Residual</b><br><i>(predicted vs observed cancer rate)</i></b></h3>";
        ...
        div.innerHTML +=
            ...
            '<label>< -2 Std. Dev. <i>(Underpredicted)</i></label>';
        ...
        div.innerHTML +=
            ...
            '<label>> 2 Std. Dev. <i>(Overpredicted)</i></label>';
